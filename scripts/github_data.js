...
const findCommits = githubData[6]["payload"]["commits"];
console.log(findCommits);
for (let i = 0; i < findCommits.length; i++) {
  if (findCommits[i].hasOwnProperty("message")) {
    console.log(findCommits[i]["message"]);
  }
};


function findCommitMessage(array) {
  let findCommits;
  let findMessage;
  for (let i = 0; i < array.length; i++) {
    if (array[i].id === "8030403992") {
      findCommits = array[i].payload.commits;
    }
  }

  for (let j = 0; j < findCommits.length; j++) {
    if (findCommits[j].hasOwnProperty("message")) {
      findMessage = findCommits[j].message
    }
  }
...
console.log(findCommitMessage(githubData));
